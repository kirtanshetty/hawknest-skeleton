...
LD65_NOCRT_CFG = $(LD65_CFG_DIR)/hawknest_nocrt.cfg
...
CTESTS = $(addprefix $(BIN_DIR)/,$(patsubst %.c,%,$(filter %.c, $(TEST_SRC))))
ASMTESTS = $(addprefix $(BIN_DIR)/,$(patsubst %.s,%,$(filter %.s, $(TEST_SRC))))
...
tests: $(CTESTS) $(ASMTESTS)

check: 
	@echo "ASMOBJ: $(TEST_ASMOBJ)"
	@echo "CTESTS: $(CTESTS)"
	@echo "ASMTESTS: $(ASMTESTS)"


clean:
	@rm -rf $(BUILD_DIR) $(BIN_DIR) submission.tar.gz

.PHONY: emu lib tests clean run-unit-tests
...
$(CTESTS): $(BIN_DIR)/%: $(TEST_BUILD_DIR)/%.o $(LIB) $(LD65_CFG)
	@echo "Linking $@"
	@mkdir -p $(dir $@)
	@$(LD65_EXEC) $< --lib $(LIB) -C $(LD65_CFG) -o $@

$(ASMTESTS): $(BIN_DIR)/%: $(TEST_BUILD_DIR)/%.o $(LD65_NOCRT_CFG)
	@echo "Linking $@"
	@mkdir -p $(dir $@)
	@$(LD65_EXEC) $< -C $(LD65_NOCRT_CFG) -o $@
